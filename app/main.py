...
async def upload(
    ...
    # ------------------------------------------------------------
    # IMPORTANT: this ID is used everywhere (UI + RQ + Redis)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # CRITICAL FIX: force RQ to use *our* job_id
    # ------------------------------------------------------------
    q = get_queue()
    q.enqueue(
        process_job,
        ...
        job_timeout=60 * 60 * 3,
        job_id=job_id,              # RQ job ID == UI job ID
    )
...
def download_output(job_id: str, file_key: str):
    ...
    decoded = {k.decode(): v.decode() for k, v in data.items()}
    index = json.loads(decoded.get("result_index", "[]"))

    try:
        i = int(file_key)
        path = index[i]["docx_path"]
    except Exception:
